<div class="flex">
    ...
    <form action="{{ "/tweets/{$tweet->id}/like" }}" method="POST">
        ...
        <div class="flex items-center mr-4 {{ $tweet->isLikedBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500' }}">
            ...
            <button type='submit' class="text-sm">{{ $tweet->likes }}</button>
        </div>
    </form>
    ...
    <form action="{{ "/tweets/{$tweet->id}/like" }}" method="POST">
        ...
        <div class="flex items-center {{ $tweet->isDislikedBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500' }}">
            <svg class='mr-1 w-3 mt-1' viewBox="0 0 20 20" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                    <g class='fill-current' id="icon-shape">
            ...
            <button type='submit' class="text-sm">{{ $tweet->dislikes }}</button>
        </div>
    </form>
